...
namespace SystemTasks
{
    public class PurchaseOrderService : IPurchaseOrderService
    {
        ...
        public List<PurchaseModel> mapPurchaseOrderHeaders(List<PurchaseOrderFile> payload)
        {
            var orders = new List<PurchaseModel>();

            foreach (var item in payload.GroupBy(u => new { u.OrderNumber, u.OrderDate, u.Notes, u.VendorName, u.ShippedToSite }))
            {
                PurchaseModel order = new PurchaseModel();
                ...
                order.PurchaseOrderDetails = new List<PurchaseItemDetailModel>();

                foreach (var det in details)
                {
                    order.PurchaseOrderDetails.Add(new PurchaseItemDetailModel
                    {
