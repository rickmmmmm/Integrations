...
AS
    BEGIN
        ...
        FROM [Configurations] 
        WHERE ConfigurationName = 'DefaultManufacturer' AND ProcessUid = @ProcessUid
          AND Enabled = 1;
        ...
        INNER JOIN (
            ...
            WHERE
                SourceManufacturer.ManufacturerName IS NOT NULL
            AND TargetManufacturer.ManufacturerName IS NOT NULL
            AND TargetManufacturer.ProcessTaskUID = @ProcessTaskUid
            AND (TargetManufacturer.Rejected = 0 OR @AllowStackingErrors = 1)
            GROUP BY
        ...
        IF @CreateManufacturers = 1
            BEGIN
                ...
                FROM
                    IntegrationMiddleWay.dbo._ETL_Inventory TargetManufacturer
                WHERE
                    TargetManufacturer.ManufacturerName IS NULL 
                 OR LTRIM(RTRIM(TargetManufacturer.ManufacturerName)) = ''
                AND TargetManufacturer.ProcessTaskUID = @ProcessTaskUid
                AND (TargetManufacturer.Rejected = 0 OR @AllowStackingErrors = 1);
            END
        ELSE
